...
function saldoAdOggi(movs){
  return movs
    ...
    .reduce((a,m)=>a+(m.tipo==="entrata"?m.importo:-m.importo),0);
}

function renderSaldoTotale(){
  let t=0;
  appState.wallets.forEach(w=>{
    t+=saldoAdOggi(w.movimenti);
  });
...
function renderGrafici(){
  const wId = walletChartSelect.value || "total";
  const wallet = wId==="total"
    ? {movimenti: appState.wallets.flatMap(w=>w.movimenti), traguardo:1000}
    : appState.wallets.find(w=>w.id===wId);

  const movs = wallet.movimenti.filter(m=>m.data<=TODAY);
  ...
  doughnut = new Chart(graficoPercentuali,{
    ...
    data:{
      ...
      datasets:[{
        data:[entrate,spese,wallet.traguardo],
        backgroundColor:["#009246","#CE2B37","#FACC15"]
      }]
    }
  });
  ...
  const values=days.map(d=>{
    movs.filter(m=>m.data===d).forEach(m=>{
...
walletChartSelect.onchange = renderGrafici;
